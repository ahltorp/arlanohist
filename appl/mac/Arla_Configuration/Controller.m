...
- (void) initCacheValues {
    int64_t maxbytes, curbytes, maxfiles, curfiles, minbytes, minfiles;
...
- (IBAction)save:(id)sender {
	...
	if ([minBytes intValue] <= 0) {
	    NSBeginAlertSheet(@"Error", @"OK", nil, nil, mainWindow,
			      nil, nil, nil, NULL,
			      @"Minimum bytes must be positive");
	    return;
	}
	if ([maxFiles intValue] <= 0) {
	    NSBeginAlertSheet(@"Error", @"OK", nil, nil, mainWindow,
			      nil, nil, nil, NULL,
			      @"Maximum files must be positive");
	    return;
	}
	if ([minFiles intValue] <= 0) {
	    NSBeginAlertSheet(@"Error", @"OK", nil, nil, mainWindow,
			      nil, nil, nil, NULL,
			      @"Minimum files must be positive");
	    return;
	}


	if ([maxBytes intValue] < [minBytes intValue]) {
	    NSBeginAlertSheet(@"Error", @"OK", nil, nil, mainWindow,
			      nil, nil, nil, NULL,
			      @"Maximum bytes must be more than minimum");
	    return;
	}

	if ([maxFiles intValue] < [minFiles intValue]) {
	    NSBeginAlertSheet(@"Error", @"OK", nil, nil, mainWindow,
			      nil, nil, nil, NULL,
			      @"Maximum files must be more than minimum");
	    return;
	}


        status = [dataSource saveConfData: gAuthorization
			     maxBytes: [maxBytes intValue] * 1024 * 1024LL
			     minBytes: [minBytes intValue] * 1024 * 1024LL
...
- (IBAction)defaultButton:(id) sender {
    ...
    [maxBytes setIntValue: 1400];
    [minBytes abortEditing];
    [minBytes setIntValue: 700];
    [maxFiles abortEditing];
    [maxFiles setIntValue: 40000];
    [minFiles abortEditing];
    [minFiles setIntValue: 30000];
    [self confChanged];
}
